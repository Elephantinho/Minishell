...
/*   Updated: 2025/05/15 01:36:26 by ale              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_heredocs(t_token **tokens, int savefd, int original_fd_in, int original_fd_out)
{
	t_token *curr = *tokens;
	t_token *prev = NULL;
	int		fd;
	...
	close(save_fd_out);//credo vada qui
	return (heredoc_done);
}
