...
/*   Updated: 2025/05/27 20:28:14 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_if_arg_of_exit_is_num_no_pipes(char ***args, t_exec_no_pipes *st, char **env, int j)
{
	j = 0;
		...
		((args[0][1][j] == '-' || args[0][1][j] == '+') && !(args[0][1][j + 1])))
	{
		...
		free_for_exit_no_pipes(args, env, st);//da lasciare solo se poi esce
		return (0);
	}
	j++;
...
void	ft_exit_no_pipes(char ***args, t_exec_no_pipes *st, char **env, int *exit_code)
{
	char	ex_code;
...
int	check_if_arg_of_exit_is_numeric(char ***args, t_token **arr_of_lists, char **env, int j)
{
	j = 0;
	if ((!(args[0][1][j] >= '0' && args[0][1][j] <= '9') && \
		(args[0][1][j] != '-' && args[0][1][j] != '+')) ||
		((args[0][1][j] == '-' || args[0][1][j] == '+') && !(args[0][1][j + 1])))
	{
		...
		write(2, args[0][1], ft_strlen(args[0][1]));
		write(2, ": numeric argument required\n", 28);
		free_for_exit(args, arr_of_lists, env);//da lasciare solo se poi esce
		return (0);
	}
	j++;
	while (args[0][1][j] && args[0][1][j] != '\0')
...
void	ft_exit(char ***args, t_token **arr_of_lists, char **env, int *exit_code)
{
	char	ex_code;
