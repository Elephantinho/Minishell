...
/*   Updated: 2025/05/21 23:21:35 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*create_arr(char *s, char **env)
{
	...
	if (!arr)
	{
		...
		free(s);
		free_env(env);
		close_basic_fds_and_exit(1);//non so se error code su exit va bene e anche se va bene exit o meglio solo return
	}
	i = 0;
	while (s[i] != '\0')
	{
		...
		t_token **token_list, char *s, int *arr)
{
	...
	if ((s[strct->i] == 32 || (s[strct->i] >= 9 && \
		...
		create_token_base_case(strct, token_list);
	else if (arr[strct->i] == 1 || arr[strct->i] == 2 || \
			(arr[strct->i] == 0 && (s[strct->i] != '|' && \
			s[strct->i] != '>' && s[strct->i] != '<')))
		base_case(strct);
	else if (s[strct->i] == '>' && s[strct->i + 1] == '>' && arr[strct->i] == 0)
		append_case(strct, token_list);
	else if (s[strct->i] == '>' && arr[strct->i] == 0)
		redirect_output_case(strct, token_list);
	else if (s[strct->i] == '<' && s[strct->i + 1] == '<' && arr[strct->i] == 0)
		heredoc_case(strct, token_list);
	else if (s[strct->i] == '<' && arr[strct->i] == 0)
		redirect_input_case(strct, token_list);
	else if (s[strct->i] == '|' && arr[strct->i] == 0)
		pipe_case(strct, token_list);
...
t_token	*separete_in_tokens(char *s, int *arr)
{
	...
	token_list = NULL;
	while ((s[strct.i] == 32 || (s[strct.i] >= 9 && s[strct.i] <= 13)))
	{
		(strct.i)++;
		strct.start = strct.i;
	}
