...
/*   Updated: 2025/05/16 00:46:45 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	int		fd_in = -1;
	...
	while(tokens[count])
	{
		...
		heredoc_alredy_done = handle_heredocs(&(tokens[count]), &fd, *env);
		heredoc_pipes[0][count] = fd;
		heredoc_pipes[1][count] = heredoc_alredy_done;
		count++;
	}
	...
	while (tokens[count]) // Ciclo per eseguire ogni comando
	{
		if (tokens[count]->e_tk_type == 0) // Se è un comando da mettere anche se e' un redirect(?)
		{
			...
			if (pids[i] == 0)
			{
				...
				/* else if (prevpipe != -1 && heredoc_alredy_done == 1)
				{
					close(prevpipe);
				} */
			...
			//	handle_redirections(tokens, heredoc_alredy_done);
				...
				built_in_or_execute(env, &tokens[count], status, save_fd_in, save_fd_out, heredoc_alredy_done);
