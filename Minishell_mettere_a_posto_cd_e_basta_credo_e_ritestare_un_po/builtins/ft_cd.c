...
/*   Updated: 2025/05/30 15:30:15 by ale              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cd_null_path(char ***envp, int *exit_code, int is_inside_pipe)
{
	...
	if (!home)
	{
		return (print_home_not_set(exit_code, is_inside_pipe));
	}
...
int	ft_cd_inital_checks(char *path, char ***envp, \
		...
		int *exit_code, int is_inside_pipe)
{
	...
	if (ft_cd_inital_checks(path, envp, exit_code, is_inside_pipe) == 1)
		return ;
	else
	{
		old_pwd = getcwd(NULL, 0);
