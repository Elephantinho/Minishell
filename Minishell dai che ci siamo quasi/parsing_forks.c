/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parsing_forks.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ade-ross <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/22 21:22:59 by ale               #+#    #+#             */
/*   Updated: 2025/05/21 23:28:18 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "includes/minishell.h"

void	second_fork_failed(char *s, int *arr, char **env, int exit_code)
{
	free(arr);
	free (s);
	free_env(env);
//	printf("error_code1111: %d\n", error_code);
	close_basic_fds_and_exit(exit_code); //da modificare error code
}

void	second_fork_worked(char *s, int *arr, char **env, t_token **tokens)
{
//	char	***arr_of_matrx;

	*tokens = separete_in_tokens(s, arr);
//	ft_lstiter_tok(*tokens , prnt);
	free(arr);
	free (s);
	check_ambig_redirect(tokens, env);
	expand_dollar(tokens, env);
//	printf("\n");
//	arr_of_matrx =  create_array_of_matrix(NULL, env, *tokens);//da ;evare
//	ft_lstiter_tok(*tokens , prnt);
	ft_lstclear_tok(tokens);
//	if (arr_of_matrx)
//		free_cmnds(arr_of_matrx); //da levare
	free_env(env);
	close_basic_fds_and_exit(0);
}

t_token	**first_fork_worked(char *s, char **env)
{
//	char	***arr_of_matrx;
	int		*arr;
	t_token *tokens;
	t_token	**arr_of_lists;

	arr = create_arr(s, env);
	tokens = separete_in_tokens(s, arr);
	free(arr);
	check_ambig_redirect(&tokens, env);
	expand_dollar(&tokens, env);
	//printf("\n");//da levare
	//ft_lstiter_tok(tokens , prnt);//sarà da levare stampa solo la lista dei token

	//arr_of_matrx = create_array_of_matrix(s, env, tokens);//da non fare piu e fare dentro a ogni pipe la sua matrice
	arr_of_lists = create_arr_of_lists(s, env, &tokens);

	int i = 0;
	while(arr_of_lists[i] != NULL)
	{
		printf("lista %d\n", i);
		ft_lstiter_tok(arr_of_lists[i], prnt);
		i++;
	}
	//da levare questo doppio ciclo for che stampa la matrice
/* 	for (int z = 0; z < 2; z++)
	{
		for (int y = 0; y < 3; y++)
			printf("s[%d][%d]: %s \n", z, y, arr_of_matrx[z][y]);
	} */
//	ft_lstclear_tok_mod(&tokens);//non va piu bene perche la lista
	//non va piu liberata ora che si fa l?arr_of lists
	//e andra ritornato larray of list non arr_of matrx che non va piu fatto
	return (arr_of_lists); //da cambiare ritornando arr_of_lists
}

void	second_fork_parsing(char *s, int *arr, char **env, t_token **tokens)
{
	free_env(env);
	*tokens = separete_in_tokens(s, arr);
	free(arr);
	free (s);
	ft_lstclear_tok(tokens);
	close_basic_fds_and_exit(0);
}

void	first_fork_parsing(char *s, char **env)
{
	int	*arr;
	int	pid2;
	int	exit_code_pars;
	t_token *tokens;

	arr = create_arr(s, env);
	pid2 = fork();
	if (pid2 == -1)
		perror("fork failed");//non so se va bene come errore
	if (pid2 == 0)
		second_fork_parsing(s, arr, env, &tokens);
	else
	{
		waitpid(pid2, &exit_code_pars, 0);
	//	printf("error_codesecfork: %d\n", exit_code_pars);
		exit_code_pars = transl_exit_code(exit_code_pars);
		if (exit_code_pars != 0)
			second_fork_failed(s, arr, env, exit_code_pars);
		else
			second_fork_worked(s, arr, env, &tokens);
	}
}
