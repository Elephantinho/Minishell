...
/*   Updated: 2025/05/23 22:14:46 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**built_in_or_execute(char ***env, t_token **tokens, int *exit_code)
{
	...
	cmnds = NULL;
	save_fd_in = dup(STDIN_FILENO);// mi sa che si puo levare questo on fa niente
	...
	if (cmnds && cmnds[0])
	{
		if ((ft_strncmp(cmnds[0], "export", 6) == 0) && cmnds[0][6] == '\0')
			...
			ft_exit(&cmnds, NULL, *env, exit_code);// da testare meglio di sicuro ci sono ancora fd da chiudere
		else
...
void	init_hereoc_pipes(t_exec *st)
{
	...
	i = 0;
	while (i < 256)//non so nemmeno se serve o forse meglio mettere  -1
	{
							...
							char ***env, int *status)
{
	...
	if (st->prevpipe != -1 || st->heredoc_alredy_done == 1)//non l'ha levato tutto questo if e il uo contenuto mi sa
	{
...
void	upd_pipe_in_exec(t_token **arr_of_lists, t_exec *st)
{
	...
	if (st->heredoc_alredy_done == 1)
		dup2(st->save_fd_in, STDIN_FILENO);//non credo serva questo forse
}
