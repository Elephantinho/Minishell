...
/*   Updated: 2025/04/20 19:17:40 by ale              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

void	create_token_base_case(t_str_tk_division *strct, \
						...
						char *s, int *arr)
{
	strct->i = strct->i + 2;
	if (strct->prev_is_whitespace == 0)
		create_token(ft_substr(s, strct->start, strct->i - 2 - strct->start), \
								token_list, NORMAL, arr);
								...
								char *s, int *arr)
{
	(strct->i)++;
	if (strct->prev_is_whitespace == 0)
		create_token(ft_substr(s, strct->start, strct->i - 1 - strct->start), \
								token_list, NORMAL, arr);
