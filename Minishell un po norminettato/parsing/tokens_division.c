...
/*   Updated: 2025/04/19 21:23:36 by ale              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sep_in_tk_lots_of_ifs(t_str_tk_division *strct, t_token **token_list, char *s, int *arr)
{
	check_quotes(s, strct->i, &(strct->quotes_open));
	if ((s[strct->i] == 32 || (s[strct->i] >= 9 && s[strct->i] <= 13)) && arr[strct->i] == 0)
...
t_token	*separete_in_tokens(char *s, int *arr)
{
	...
	else if (!((s[strct.i - 1] == 32 || (s[strct.i - 1] >= 9 && s[strct.i - 1] <= 13)) && arr[strct.i - 1] == 0))
		create_token(ft_substr(s, strct.start, strct.i - strct.start), &token_list, NORMAL, arr);
	return (token_list);
}
