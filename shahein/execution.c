...
/*   Updated: 2025/05/13 21:57:31 by mshahein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execution(t_token **tokens, char ***env)
{
	...
	{
		if (tokens[count]->e_tk_type != PIPE) // Se è un comando
		{
			...
			if (pids[i] == 0)
			{
				...
				// Esegui il comando
				handle_heredocs(&tokens[count], save_stdin);
				printf("executing command: %s\n with input from heredoc\n", tokens[count]->token);
