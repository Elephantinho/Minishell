...
/*   Updated: 2025/05/12 16:48:00 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_if_arg_of_exit_is_numeric(char ***args, char *s, char **env, int j)
{
		...
		args[0][1][j] != '-' && args[0][1][j] != '+')
	{
		...
		free_for_exit(args, s, env);//da lasciare solo se poi esce
		return (0);
	}
	j++;
...
{
	char	ex_code;
	//controlla valgrind ma in realta dovrebbe essere a posto
	write(2, "exit\n", 5);
	if (args[0][1] && args[0][1][0] && args[0][1][0] != '\0')
	{
		if (check_if_arg_of_exit_is_numeric(args, s, env, 0) == 0)
			exit(2);// mi pare esca anche in questo caso quasi sicuro
		if (args[0][2] != NULL)
		{
			*exit_code = 256;
	...
	{
		//!!!da modificare perche caso base non ritorna sempre ma ritorna l'ultimo exit_code
		free_for_exit(args, s, env);
		exit(0);
	}
}
