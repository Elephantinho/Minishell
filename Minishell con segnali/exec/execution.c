...
/*   Updated: 2025/05/19 23:12:14 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	int		fd_in = -1;
	...
	while(arr_of_lists[count])
	{
		...
		if (g_exit_status == 130)
		{
			...
			{
				if (heredoc_pipes[1][l] == 1)
				{
				//	printf("process : %d, waaaaaaaaaaaaaaaaaa\n", count);
						close(heredoc_pipes[0][l]);
				}
				l++;
			}
			t_token *temp;
	...
	count = 0;
	while (arr_of_lists[count] && g_exit_status == 0) // Ciclo per eseguire ogni comando
	{
