...
/*   Updated: 2025/05/22 19:32:08 by ade-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_heredoc(char *limiter, char **env)
{
	...
	while (1)
	{
		...
		if (!line)
		{
			...
			if(g_exit_status == 0)
				;// da aggiungere messaggio d'errore come quello di bash scrivendolo sul std err con write(2, ...)
			break;
		}
		...
		if (curr->e_tk_type == HEREDOC && curr->next && curr->next->e_tk_type == NORMAL)
		{
			...
			*fd = handle_heredoc(curr->next->token, env);
			...
			if (*fd != -1)
			{
				...
				//questo e' il caso in cui la pipe fallisce
			}
			else
	...
	while (curr)
	{
			...
			curr->next && curr->next->e_tk_type == NORMAL)
		{
			...
			if (fd == -1)
			{
				...
				write(2, ": ", 2);
				perror("redirection error");//scrive errore fatto bene sia printa permission denied sia no such file or directory e fare exit
				return (0);
			}
			else
				close(fd);
