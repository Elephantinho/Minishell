...
/*   Updated: 2025/05/18 02:37:25 by ale              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**built_in_or_execute(char ***env, t_token **arr_of_lists, int *exit_code, int original_fd_in, int original_fd_out, int heredoc_alredy_done)
{
	char	**cmnds;
	int		save_fd_in;
//	int		save_fd_out;
...
//	printf("curr: %s\n", (*arr_of_lists)->token);
	//heredoc_alredy_done = handle_heredocs(arr_of_lists, save_fd_in, original_fd_in, original_fd_out);
	...
	cmnds = create_matrix_and_free_arr_of_lists(arr_of_lists, arr_of_lists, &cmnds);
	//printf("hooo\n");
	print_matrix(cmnds);
	if (cmnds && cmnds[0])
	{
...
//	int		fd_in = -1;
	...
	count = 0;
	while (arr_of_lists[count]) // Ciclo per eseguire ogni comando
	{
		/* if (arr_of_lists[count]->e_tk_type == 0) // Se è un comando da mettere anche se e' un redirect(?)
		{ */
			...
			//	printf("arr_of_lists[%d]->token: %s\n", count, (arr_of_lists[count])->token);
				...
				{
					t_token *prev = NULL;
					t_token *curr = arr_of_lists[count];
					//temp = &(arr_of_lists[count]);
					while(curr)
					{
						if (curr->e_tk_type == HEREDOC && curr->next && curr->next->e_tk_type == NORMAL)
						{
							remove_redir_tokens(&(arr_of_lists[count]), prev, curr);
							if (prev == NULL)
								curr = arr_of_lists[count];
							else
								curr = prev->next;
						}
						else
